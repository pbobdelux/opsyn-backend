...
async def mobile_orders_paginated(
    ...
    logger.info(
        "[MOBILE_ORDERS_PAGE_RETURNED] brand_id=%s limit=%s cursor=%s",
        brand_id,
        limit,
    ...
    if cursor:
        try:
            ...
            logger.error(
                "[MOBILE_ORDERS_PAGE_RETURNED] cursor_decode_error brand_id=%s error=%s",
                brand_id,
                cursor_exc,
            )
    ...
    # ------------------------------------------------------------------
    # Build query — scoped by brand_id, ordered by updated_at DESC
    # ------------------------------------------------------------------
    try:
        query = (
            select(Order)
        ...
        if cursor_id and cursor_updated_at:
            query = query.where(
                (Order.updated_at < cursor_updated_at)
                | ((Order.updated_at == cursor_updated_at) & (Order.id < cursor_id))
            )
        ...
        logger.error(
            "[MOBILE_ORDERS_PAGE_RETURNED] db_query_failed brand_id=%s error=%s",
            brand_id,
            str(exc)[:300],
        )
        try:
    ...
    # ------------------------------------------------------------------
    # Total count (cheap indexed COUNT)
    # ------------------------------------------------------------------
    total_count = 0
    try:
        count_res = await db.execute(
        ...
        logger.error(
            "[MOBILE_ORDERS_PAGE_RETURNED] count_failed brand_id=%s error=%s",
            brand_id,
            str(exc)[:200],
        )
        try:
    ...
    if has_more and orders:
        last_order = orders[-1]
        sort_val = getattr(last_order, "updated_at", None)
    ...
    # ------------------------------------------------------------------
    # Serialize — compact fields only, no huge nested payloads
    # ------------------------------------------------------------------
    ...
    serialized_orders = [
        {
            ...
            "delivery_status": _safe(o, "delivery_status", "pending"),
            "payment_status": _safe(o, "payment_status", "unpaid"),
            "balance_due": float(_safe(o, "balance_due", 0) or 0),
            "delivery_date": _safe(o, "delivery_date"),
    ...
    logger.info(
        "[MOBILE_ORDERS_PAGE_RETURNED] brand_id=%s returned=%s total=%s"
        " has_more=%s limit=%s original_limit=%s",
        brand_id,
