...
async def reprocess_dead_letters(
    ...
    Logs: [REPROCESS_STARTED], [REPROCESS_COMPLETE]
    """
    from database import AsyncSessionLocal
    from services.leaflink_sync import sync_leaflink_orders

    logger.info("[REPROCESS_STARTED] brand_id=%s limit=%s source=reprocess-dead-letters", brand_id, limit)
    ...
    if not rows:
        ...
        return {"ok": True, "brand_id": brand_id, "attempted": 0, "succeeded": 0, "failed": 0, "message": "No unresolved dead-letter records found"}
    ...
    for row in rows:
        ...
        try:
            ...
            if inserted > 0 or updated > 0:
                await db.execute(
                    text("UPDATE sync_dead_letters SET resolved_at = :now, retry_count = retry_count + 1 WHERE id = :id"),
                    {"now": now, "id": record_id},
                )
                await db.commit()
                succeeded += 1
                logger.info("[REPROCESS_ORDER_SUCCESS] brand_id=%s external_id=%s record_id=%s", brand_id, external_id, record_id)
            else:
                await db.execute(
                ...
                logger.warning("[REPROCESS_ORDER_FAILED] brand_id=%s external_id=%s record_id=%s reason=no_rows_written", brand_id, external_id, record_id)

        except Exception as exc:
            failed += 1
            failed_ids.append(record_id)
            logger.error("[REPROCESS_ORDER_ERROR] brand_id=%s external_id=%s record_id=%s error=%s", brand_id, external_id, record_id, str(exc)[:300])
            try:
                await db.rollback()
            except Exception:
                pass

    logger.info("[REPROCESS_COMPLETE] brand_id=%s attempted=%s succeeded=%s failed=%s", brand_id, attempted, succeeded, failed)
    return {
        "ok": True,
        "warning": failed > 0,
        "brand_id": brand_id,
...
async def reprocess_order(
    ...
    """Retry a specific order by its LeafLink external_id.
    ...
    logger.info("[REPROCESS_STARTED] brand_id=%s external_id=%s source=reprocess-order", brand_id, order_id)
    now = datetime.now(timezone.utc)
    ...
    if not row:
        logger.info("[REPROCESS_COMPLETE] brand_id=%s external_id=%s attempted=0 reason=no_unresolved_record", brand_id, order_id)
        return {"ok": True, "brand_id": brand_id, "external_id": order_id, "attempted": False, "message": f"No unresolved dead-letter record found for external_id={order_id}"}
    ...
    try:
        ...
        if inserted > 0 or updated > 0:
            ...
            await db.commit()
            logger.info("[REPROCESS_COMPLETE] brand_id=%s external_id=%s record_id=%s succeeded=true", brand_id, order_id, record_id)
            return {"ok": True, "brand_id": brand_id, "external_id": order_id, "record_id": record_id, "attempted": True, "succeeded": True, "inserted": inserted, "updated": updated, "message": f"Order {order_id} reprocessed successfully"}
        else:
            await db.execute(text("UPDATE sync_dead_letters SET retry_count = retry_count + 1 WHERE id = :id"), {"id": record_id})
            await db.commit()
            logger.warning("[REPROCESS_COMPLETE] brand_id=%s external_id=%s record_id=%s succeeded=false reason=no_rows_written", brand_id, order_id, record_id)
            return {"ok": False, "brand_id": brand_id, "external_id": order_id, "record_id": record_id, "attempted": True, "succeeded": False, "message": f"Order {order_id} reprocessing attempted but no rows were written", "sync_result": reprocess_result}

    except Exception as exc:
        err_msg = str(exc)[:300]
        logger.error("[REPROCESS_COMPLETE] brand_id=%s external_id=%s record_id=%s succeeded=false error=%s", brand_id, order_id, record_id, err_msg)
        try:
            await db.rollback()
        except Exception:
            pass
        return {"ok": False, "brand_id": brand_id, "external_id": order_id, "record_id": record_id, "attempted": True, "succeeded": False, "error": err_msg, "message": f"Order {order_id} reprocessing failed: {err_msg}"}
...
async def backfill_normalized_dates(
    ...
    logger.info("[REPROCESS_STARTED] brand_id=%s limit=%s source=backfill-normalized-dates", brand_id, limit)
    now = datetime.now(timezone.utc)
    ...
    if not rows:
        logger.info("[REPROCESS_COMPLETE] brand_id=%s attempted=0 succeeded=0 source=backfill-normalized-dates reason=no_orders_with_null_dates", brand_id)
        return {"ok": True, "brand_id": brand_id, "attempted": 0, "succeeded": 0, "skipped": 0, "message": "No orders with NULL external timestamps found"}
    ...
    for row in rows:
        ...
        created_raw = raw_payload.get("created") or raw_payload.get("created_at") or raw_payload.get("external_created_at")
        modified_raw = raw_payload.get("modified") or raw_payload.get("updated") or raw_payload.get("updated_at") or raw_payload.get("external_updated_at")
        ...
        try:
            await db.execute(
                text("""
                    ...
                    WHERE id = :order_id
                """),
                {"ext_created": ext_created, "ext_updated": ext_updated, "now": now, "order_id": order_id},
            )
            await db.commit()
            succeeded += 1
        except Exception as exc:
            logger.error("[REPROCESS_DATE_ERROR] brand_id=%s order_id=%s external_order_id=%s error=%s", brand_id, order_id, external_order_id, str(exc)[:200])
            try:
                await db.rollback()
            except Exception:
                pass
    ...
    logger.info("[REPROCESS_COMPLETE] brand_id=%s attempted=%s succeeded=%s skipped=%s source=backfill-normalized-dates", brand_id, attempted, succeeded, skipped)
    return {
        "ok": True,
        "brand_id": brand_id,
        "attempted": attempted,
