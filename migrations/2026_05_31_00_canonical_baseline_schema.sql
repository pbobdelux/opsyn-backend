-- ============================================================================
-- Migration: 2026_05_31_00_canonical_baseline_schema
-- ============================================================================
-- PURPOSE
--   Establish a single, authoritative, fully-idempotent schema baseline that
--   represents the desired production state.  Every table is created with
--   IF NOT EXISTS guards so this migration is safe to run against:
--     - a completely empty database
--     - a partially-migrated database
--     - a database that already has the correct schema (pure no-op)
--
-- STRATEGY
--   All canonical application tables are created here with:
--     - UUID primary keys (gen_random_uuid() default)
--     - FK columns that match the type of the referenced PK (UUID → UUID)
--     - Proper creation order: base tables before dependent tables
--     - No prose, no debug text, no invalid SQL
--
--   Legacy/compatibility tables that use SERIAL PKs are preserved as-is
--   because downstream code still references them by integer ID.
--
-- QUARANTINE RELATIONSHIP
--   The following migrations are quarantined in SKIPPED_LEGACY_MIGRATIONS
--   and will never be applied.  This migration supersedes them:
...
--   1.  organizations, brands, employees, employee_* (auth — no FK deps)
--   2.  brand_api_credentials (no FK deps)
--   3.  sync_runs (UUID PK — orders.sync_run_id references this)
--   4.  sync_requests (UUID PK — no FK deps)
--   5.  drivers (UUID PK — routes.assigned_driver_id and orders.assigned_driver_id reference this)
--   6.  routes (UUID PK — references drivers; orders.route_id references this)
--   7.  route_stops (UUID PK — references routes)
--   8.  orders (UUID PK — references sync_runs, drivers, routes)
--   9.  order_lines (UUID PK — references orders)
--   10. sync_health (no FK deps)
--   11. sync_dead_letters (no FK deps)
--   12. sync_metrics_snapshots (no FK deps)
--   13. dead_letter_line_items (references orders)
--   14. organization_brand_bindings (no FK deps)
--   15. tenant_credentials (no FK deps)
--   16. route_events (UUID PK — references routes)
--   17. driver_locations (UUID PK — references drivers, routes)
--   18. driver_route_history (UUID PK — references drivers, routes, route_stops)
--   19. assistant_sessions, assistant_messages, assistant_pending_actions,
--       assistant_audit_logs (VARCHAR(36) PKs — match ORM models)
--   20. leaflink_webhook_events (UUID PK — self-referencing FK)
-- ============================================================================

-- ============================================================================
-- 1. AUTH TABLES (VARCHAR PKs — match 2026_05_04_01_setup_auth_tables.sql)
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_employees_org_id
    ON employees (org_id);
CREATE INDEX IF NOT EXISTS idx_employee_passcodes_employee_id
    ON employee_passcodes (employee_id);
CREATE INDEX IF NOT EXISTS idx_employee_brand_access_employee_id
    ON employee_brand_access (employee_id);
CREATE INDEX IF NOT EXISTS idx_employee_app_access_employee_id
    ON employee_app_access (employee_id);

-- ============================================================================
-- 2. BRAND API CREDENTIALS (UUID PK)
-- ============================================================================
...
-- ============================================================================
-- 3. SYNC RUNS (UUID PK — must exist before orders)
-- ============================================================================
...
-- ============================================================================
-- 5. DRIVERS (UUID PK — must exist before routes and orders)
-- ============================================================================
...
-- ============================================================================
-- 6. ROUTES (UUID PK — references drivers; must exist before route_stops,
--    route_events, driver_locations, driver_route_history, and orders)
-- ============================================================================
...
-- ============================================================================
-- 7. ROUTE STOPS (UUID PK — references routes)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS orders (
    ...
    CONSTRAINT ck_orders_delivery_status
...
CREATE INDEX IF NOT EXISTS ix_orders_customer_name
...
-- ============================================================================
-- 9. ORDER LINES (UUID PK — references orders)
-- ============================================================================
...
-- ============================================================================
-- 12. SYNC METRICS SNAPSHOTS (UUID PK)
-- ============================================================================
...
-- ============================================================================
-- 13. DEAD LETTER LINE ITEMS (UUID PK — references orders)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS dead_letter_line_items (
    ...
    created_at          TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_dead_letter_brand_order_sku
...
-- ============================================================================
-- 14. ORGANIZATION BRAND BINDINGS (UUID PK)
-- ============================================================================
...
-- ============================================================================
-- 16. ROUTE EVENTS (UUID PK — references routes)
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS ix_route_events_route_id
    ON route_events (route_id);
CREATE INDEX IF NOT EXISTS ix_route_events_org_id
    ON route_events (org_id);
CREATE INDEX IF NOT EXISTS ix_route_events_created_at
    ON route_events (created_at);

-- ============================================================================
-- 17. DRIVER LOCATIONS (UUID PK — references drivers, routes)
-- ============================================================================
...
-- ============================================================================
-- 18. DRIVER ROUTE HISTORY (UUID PK — references drivers, routes, route_stops)
-- ============================================================================
...
-- ============================================================================
-- 19. ASSISTANT TABLES (VARCHAR(36) PKs — match ORM models in assistant_models.py)
-- ============================================================================
...
-- ============================================================================
-- 20. LEAFLINK WEBHOOK EVENTS (UUID PK — self-referencing FK)
-- ============================================================================
...
-- MIGRATION COMPLETE
-- ============================================================================
-- Canonical baseline schema established.
-- All tables created with IF NOT EXISTS — safe to run on any database state.
-- UUID PKs on all canonical app tables; VARCHAR PKs on auth/assistant tables
-- to match ORM models.
-- FK types match referenced PK types throughout.
